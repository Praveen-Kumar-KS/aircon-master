...
namespace Aircon.TagHelpers
{
    ...
    [HtmlTargetElement("air-grid-edit-contact", Attributes = CLASS_ID_ATTRIBUTE_NAME + "," + PARTIAL_ATTRIBUTE_NAME, TagStructure = TagStructure.WithoutEndTag)]
    public class AirGridEditContactTagHelper : TagHelper
    {
        #region Constants
        ...
        public AirGridEditContactTagHelper(IHtmlGenerator generator, IHtmlHelper htmlHelper)
        {
            Generator = generator;
        ...
        public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
        {
            ...
            var modalId = await new HtmlString(modelName + "-edit-partial-" + ClassId).RenderHtmlContentAsync();
            ...
            script.InnerHtml.AppendHtml(
                "$(document).ready(function () {" +
                    ...
                    "var modal_contactId_value = $(e.relatedTarget).attr('data-id'); " +
                    "var modal_addressId_value = $(e.relatedTarget).attr('data-id'); " +
                    $"$('#{modalContentId}').load('{Partial}?id=' + modal_id_value +'&contactId=' + modal_contactId_value + '&addressId=' + modal_addressId_value);" +
                    "})" +
                "});");
